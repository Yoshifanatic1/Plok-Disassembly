...
!Define_PLOK_Global_DisableCopyDetection = !FALSE
	...
	%SetROMToAssembleForHack(PLOK_G, !ROMID)
endmacro
...
macro PLOK_GlobalAssemblySettings()
	...
	!Define_Global_BaseROMMD5Hash = "43560f356fb0534d7e0e25aba1ca31e5"
...
warn "The German version of Plok is not 100% complete. Glitches may occur if this is loaded in an emulator."
...
Main:
	...
	%InsertGarbageData(NULLROM, incbin, UNK_9EFBF9_PLOK_G.bin)
	%DATATABLE_RT01_PLOK_GermanTextStrings($9F8000)
	%DATATABLE_PLOK_SecondaryLanguageBigLetterTileDataTable($9FA5C7)
	%DATATABLE_PLOK_LanguageSelectScreenText($9FA667)
	%InsertGarbageData(NULLROM, incbin, UNK_9FA695_PLOK_E.bin)
check bankcross on
	%FREE_BYTES(NULLROM, 92, $FF)
%BANK_END(!BANK_1F)
endmacro
